...
<body>
  ...
  <main>
    ...
    <div id="tab-relays" class="tab-content active">
      ...
      <div class="card">
        ...
        <div class="grid-2">
          <div class="form-group">
            <label data-i18n="osc_port_label">Port UDP</label>
      ...
      <div class="card">
        <h2>📋 <span data-i18n="osc_params_title">Paramètres OSC disponibles</span></h2>
        <div class="info-box" data-i18n="osc_params_info">Ces adresses OSC sont acceptées par le firmware (UDP, port ci-dessus). Les valeurs sont : entier (i), flottant (f) ou booléen (T/F).</div>
        <div style="overflow-x:auto;">
          <table style="width:100%;border-collapse:collapse;font-size:12px;">
            <thead>
              <tr style="border-bottom:1px solid #30363d;">
                <th style="text-align:left;padding:6px 10px;color:#8b949e;" data-i18n="osc_col_addr">Adresse OSC</th>
                <th style="text-align:left;padding:6px 10px;color:#8b949e;" data-i18n="osc_col_type">Type</th>
                <th style="text-align:left;padding:6px 10px;color:#8b949e;" data-i18n="osc_col_values">Valeurs</th>
                <th style="text-align:left;padding:6px 10px;color:#8b949e;" data-i18n="osc_col_desc">Description</th>
              </tr>
            </thead>
            <tbody id="oscParamsTable">
              <tr style="border-bottom:1px solid #21262d;">
                <td style="padding:6px 10px;color:#58a6ff;font-family:monospace;">/relay/1 … /relay/8</td>
                <td style="padding:6px 10px;color:#3fb950;">i / f / T / F</td>
                <td style="padding:6px 10px;color:#c9d1d9;">0, 1 · 0.0, 1.0 · True/False</td>
                <td style="padding:6px 10px;color:#8b949e;" data-i18n="osc_desc_relay">Active / désactive le relais N</td>
              </tr>
              <tr style="border-bottom:1px solid #21262d;">
                <td style="padding:6px 10px;color:#58a6ff;font-family:monospace;">/relay/all</td>
                <td style="padding:6px 10px;color:#3fb950;">i / f / T / F</td>
                <td style="padding:6px 10px;color:#c9d1d9;">0, 1 · True/False</td>
                <td style="padding:6px 10px;color:#8b949e;" data-i18n="osc_desc_all">Active / désactive tous les relais</td>
              </tr>
              <tr style="border-bottom:1px solid #21262d;">
                <td style="padding:6px 10px;color:#58a6ff;font-family:monospace;">/ap</td>
                <td style="padding:6px 10px;color:#3fb950;">i / T / F</td>
                <td style="padding:6px 10px;color:#c9d1d9;">0 = OFF · 1 = ON</td>
                <td style="padding:6px 10px;color:#8b949e;" data-i18n="osc_desc_ap">Active / désactive le point d'accès WiFi</td>
              </tr>
              <tr>
                <td style="padding:6px 10px;color:#58a6ff;font-family:monospace;">/reboot</td>
                <td style="padding:6px 10px;color:#3fb950;">i / T</td>
                <td style="padding:6px 10px;color:#c9d1d9;">1 / True</td>
                <td style="padding:6px 10px;color:#8b949e;" data-i18n="osc_desc_reboot">Redémarre l'ESP32</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="info-box" style="margin-top:10px;font-size:11px;" data-i18n="osc_custom_note">Les adresses /relay/N sont personnalisables dans la section ci-dessous.</div>
      </div>

      <div class="card">
        <h2>⚙️ <span data-i18n="relay_osc_config">Adresses OSC des Relais</span></h2>
    ...
    <div id="tab-network" class="tab-content">
      <div class="card">
        ...
        <!-- Champs IP statiques -->
        <div style="margin-bottom:8px;font-size:11px;color:#8b949e;font-weight:600;text-transform:uppercase;letter-spacing:.05em;" data-i18n="static_ip_section">Configuration IP statique</div>
        <div class="eth-static-fields grid-2" id="ethStaticFields">
          ...
          <div class="form-group">
            <label data-i18n="dns1_label">DNS 1</label>
            <input id="ethDns1" placeholder="8.8.8.8">
          </div>
          <div class="form-group">
            <label data-i18n="dns2_label">DNS 2</label>
  ...
  <script>
    ...
    var I18N = {
      fr:{tab_relays:'Relais & OSC',tab_network:'Réseau',tab_system:'Système',relay_control:'Contrôle des Relais',osc_listen:'Écoute OSC sur port UDP:',all_on:'✅ ALL ON',all_off:'⛔ ALL OFF',relay_osc_config:'Adresses OSC des Relais',save_btn:'💾 Sauvegarder',eth_title:'Ethernet (W5500)',use_dhcp:'Utiliser DHCP',dhcp_info:'Activé : adresse IP assignée automatiquement. Désactivé : utiliser les champs ci-dessous.',hostname:'Nom d\'hôte (Hostname)',ip_addr:'Adresse IP',subnet:'Masque de sous-réseau',gateway:'Passerelle',dns1_label:'DNS 1',dns2_label:'DNS 2',static_ip_section:'Configuration IP statique',osc_port:"Port d'écoute OSC (UDP)",osc_port_label:'Port UDP',osc_params_title:'Paramètres OSC disponibles',osc_params_info:'Ces adresses OSC sont acceptées par le firmware. Les valeurs acceptées : entier (i), flottant (f) ou booléen (T/F).',osc_col_addr:'Adresse OSC',osc_col_type:'Type',osc_col_values:'Valeurs acceptées',osc_col_desc:'Description',osc_desc_relay:'Active / désactive le relais N',osc_desc_all:'Active / désactive tous les relais',osc_desc_ap:'Active / désactive le point d\'accès WiFi',osc_desc_reboot:'Redémarre l\'ESP32',osc_custom_note:'Les adresses /relay/N sont personnalisables dans la section ci-dessous.',ap_title:"Wi-Fi / Point d'Accès (AP)",ap_info:"L'AP WiFi démarre automatiquement et s'éteint après le délai configuré (0 = toujours actif).",allow_ap:"Activer le Point d'Accès",ssid_label:'SSID (nom du réseau WiFi)',password:'Mot de passe (≥8 caractères)',ap_ip:"IP du Point d'Accès",ap_mask:'Masque AP',ap_gw:'Passerelle AP',ap_timeout:'Mise en veille AP (minutes)',ap_timeout_info:'0 = toujours actif (pas de mise en veille)',qr_title:'📱 QR Code WiFi',qr_scan:'Scannez avec votre téléphone pour vous connecter',sys_title:'Informations Système',model:'Modèle :',uptime:'Uptime :',free_ram:'RAM libre :',min_ram:'RAM min :',ethernet_lbl:'Ethernet :',eth_ip:'IP Ethernet :',osc_port_sys:'Port OSC :',wifi_ap_lbl:'WiFi AP :',ap_ip_sys:'IP AP :',ap_clients:'Clients AP :',ap_sleep_lbl:'Mise en veille AP :',active_relays:'Relais actifs :',cpu_temp:'Température CPU :',actions:'Actions',reboot:"🔄 Redémarrer l'ESP32",factory_reset:"🗑️ Réinitialiser aux paramètres d'usine",changelog:'Changelog',relay:'Relais',osc_addr:'Adresse OSC',invert:'Inverser logique',mode:'Mode',mode_latch:'Latch (valeur directe)',mode_toggle:'Toggle (basculement)',config_saved:'Configuration sauvegardée',save_error:'Erreur de sauvegarde',net_error:'Erreur réseau',net_saved:'Configuration réseau sauvegardée',net_saved_reboot:'Sauvegardé — Redémarrage dans 2s…',ap_saved:'Configuration AP sauvegardée',confirm_reboot:"Redémarrer l'ESP32 ?",rebooting:'Redémarrage en cours…',confirm_factory:"Réinitialiser aux paramètres d'usine ? Cette action est irréversible.",resetting:'Réinitialisation en cours…',log_cleared:'Journal vidé',connecting:'Connexion…',connected:'✅ Connecté',disconnected:'❌ Déconnecté',ap_active:'✅ Actif',ap_inactive:'⏸️ Inactif',disabled_always:'Désactivée (toujours actif)',conn_lost:'Connexion perdue…',ram_critical:'⚠ RAM critique'},
      en:{tab_relays:'Relays & OSC',tab_network:'Network',tab_system:'System',relay_control:'Relay Control',osc_listen:'OSC listening on UDP port:',all_on:'✅ ALL ON',all_off:'⛔ ALL OFF',relay_osc_config:'OSC Addresses per Relay',save_btn:'💾 Save',eth_title:'Ethernet (W5500)',use_dhcp:'Use DHCP',dhcp_info:'Enabled: IP assigned automatically. Disabled: use static fields below.',hostname:'Hostname',ip_addr:'IP Address',subnet:'Subnet Mask',gateway:'Gateway',dns1_label:'DNS 1',dns2_label:'DNS 2',static_ip_section:'Static IP Configuration',osc_port:'OSC Listen Port (UDP)',osc_port_label:'UDP Port',osc_params_title:'Available OSC Parameters',osc_params_info:'These OSC addresses are accepted by the firmware. Accepted values: integer (i), float (f) or boolean (T/F).',osc_col_addr:'OSC Address',osc_col_type:'Type',osc_col_values:'Accepted Values',osc_col_desc:'Description',osc_desc_relay:'Enable / disable relay N',osc_desc_all:'Enable / disable all relays',osc_desc_ap:'Enable / disable WiFi Access Point',osc_desc_reboot:'Reboot the ESP32',osc_custom_note:'The /relay/N addresses can be customised in the section below.',ap_title:'Wi-Fi / Access Point (AP)',ap_info:'WiFi AP starts automatically and shuts down after the configured timeout (0 = always active).',allow_ap:'Enable Access Point',ssid_label:'SSID (WiFi network name)',password:'Password (≥8 characters)',ap_ip:'Access Point IP',ap_mask:'AP Mask',ap_gw:'AP Gateway',ap_timeout:'AP Sleep Timeout (minutes)',ap_timeout_info:'0 = always active (no sleep)',qr_title:'📱 WiFi QR Code',qr_scan:'Scan with your phone to connect',sys_title:'System Information',model:'Model:',uptime:'Uptime:',free_ram:'Free RAM:',min_ram:'Min RAM:',ethernet_lbl:'Ethernet:',eth_ip:'Ethernet IP:',osc_port_sys:'OSC Port:',wifi_ap_lbl:'WiFi AP:',ap_ip_sys:'AP IP:',ap_clients:'AP Clients:',ap_sleep_lbl:'AP Sleep:',active_relays:'Active Relays:',cpu_temp:'CPU Temperature:',actions:'Actions',reboot:'🔄 Reboot ESP32',factory_reset:'🗑️ Factory Reset',changelog:'Changelog',relay:'Relay',osc_addr:'OSC Address',invert:'Invert logic',mode:'Mode',mode_latch:'Latch (direct value)',mode_toggle:'Toggle',config_saved:'Configuration saved',save_error:'Save error',net_error:'Network error',net_saved:'Network configuration saved',net_saved_reboot:'Saved — Rebooting in 2s…',ap_saved:'AP configuration saved',confirm_reboot:'Reboot ESP32?',rebooting:'Rebooting…',confirm_factory:'Factory reset? This action is irreversible.',resetting:'Resetting…',log_cleared:'Log cleared',connecting:'Connecting…',connected:'✅ Connected',disconnected:'❌ Disconnected',ap_active:'✅ Active',ap_inactive:'⏸️ Inactive',disabled_always:'Disabled (always active)',conn_lost:'Connection lost…',ram_critical:'⚠ Critical RAM'},
      es:{tab_relays:'Relés & OSC',tab_network:'Red',tab_system:'Sistema',relay_control:'Control de Relés',osc_listen:'Escucha OSC en puerto UDP:',all_on:'✅ TODO ON',all_off:'⛔ TODO OFF',relay_osc_config:'Direcciones OSC de los Relés',save_btn:'💾 Guardar',eth_title:'Ethernet (W5500)',use_dhcp:'Usar DHCP',dhcp_info:'Activado: IP asignada automáticamente. Desactivado: usar los campos estáticos.',hostname:'Nombre de host (Hostname)',ip_addr:'Dirección IP',subnet:'Máscara de subred',gateway:'Puerta de enlace',dns1_label:'DNS 1',dns2_label:'DNS 2',static_ip_section:'Configuración IP estática',osc_port:'Puerto de escucha OSC (UDP)',osc_port_label:'Puerto UDP',osc_params_title:'Parámetros OSC disponibles',osc_params_info:'Estas direcciones OSC son aceptadas por el firmware. Valores aceptados: entero (i), flotante (f) o booleano (T/F).',osc_col_addr:'Dirección OSC',osc_col_type:'Tipo',osc_col_values:'Valores aceptados',osc_col_desc:'Descripción',osc_desc_relay:'Activa / desactiva el relé N',osc_desc_all:'Activa / desactiva todos los relés',osc_desc_ap:'Activa / desactiva el punto de acceso WiFi',osc_desc_reboot:'Reinicia el ESP32',osc_custom_note:'Las direcciones /relay/N son personalizables en la sección siguiente.',ap_title:'Wi-Fi / Punto de Acceso (AP)',ap_info:'El AP WiFi se inicia automáticamente y se apaga tras el tiempo configurado (0 = siempre activo).',allow_ap:'Activar Punto de Acceso',ssid_label:'SSID (nombre de red WiFi)',password:'Contraseña (≥8 caracteres)',ap_ip:'IP del Punto de Acceso',ap_mask:'Máscara AP',ap_gw:'Puerta de enlace AP',ap_timeout:'Tiempo de espera AP (minutos)',ap_timeout_info:'0 = siempre activo (sin suspensión)',qr_title:'📱 Código QR WiFi',qr_scan:'Escanee con su teléfono para conectarse',sys_title:'Información del Sistema',model:'Modelo:',uptime:'Uptime:',free_ram:'RAM libre:',min_ram:'RAM mín:',ethernet_lbl:'Ethernet:',eth_ip:'IP Ethernet:',osc_port_sys:'Puerto OSC:',wifi_ap_lbl:'WiFi AP:',ap_ip_sys:'IP AP:',ap_clients:'Clientes AP:',ap_sleep_lbl:'Suspensión AP:',active_relays:'Relés activos:',cpu_temp:'Temperatura CPU:',actions:'Acciones',reboot:'🔄 Reiniciar ESP32',factory_reset:'🗑️ Restablecer valores de fábrica',changelog:'Registro de cambios',relay:'Relé',osc_addr:'Dirección OSC',invert:'Invertir lógica',mode:'Modo',mode_latch:'Latch (valor directo)',mode_toggle:'Toggle (alternancia)',config_saved:'Configuración guardada',save_error:'Error al guardar',net_error:'Error de red',net_saved:'Configuración de red guardada',net_saved_reboot:'Guardado — Reiniciando en 2s…',ap_saved:'Configuración AP guardada',confirm_reboot:'¿Reiniciar ESP32?',rebooting:'Reiniciando…',confirm_factory:'¿Restablecer valores de fábrica? Esta acción es irreversible.',resetting:'Reinicializando…',log_cleared:'Registro borrado',connecting:'Conectando…',connected:'✅ Conectado',disconnected:'❌ Desconectado',ap_active:'✅ Activo',ap_inactive:'⏸️ Inactivo',disabled_always:'Desactivado (siempre activo)',conn_lost:'Conexión perdida…',ram_critical:'⚠ RAM crítica'},
      de:{tab_relays:'Relais & OSC',tab_network:'Netzwerk',tab_system:'System',relay_control:'Relaissteuerung',osc_listen:'OSC-Empfang auf UDP-Port:',all_on:'✅ ALLE AN',all_off:'⛔ ALLE AUS',relay_osc_config:'OSC-Adressen der Relais',save_btn:'💾 Speichern',eth_title:'Ethernet (W5500)',use_dhcp:'DHCP verwenden',dhcp_info:'Aktiviert: IP wird automatisch zugewiesen. Deaktiviert: statische Felder unten verwenden.',hostname:'Hostname',ip_addr:'IP-Adresse',subnet:'Subnetzmaske',gateway:'Gateway',dns1_label:'DNS 1',dns2_label:'DNS 2',static_ip_section:'Statische IP-Konfiguration',osc_port:'OSC-Empfangsport (UDP)',osc_port_label:'UDP-Port',osc_params_title:'Verfügbare OSC-Parameter',osc_params_info:'Diese OSC-Adressen werden vom Firmware akzeptiert. Akzeptierte Werte: Integer (i), Float (f) oder Boolean (T/F).',osc_col_addr:'OSC-Adresse',osc_col_type:'Typ',osc_col_values:'Akzeptierte Werte',osc_col_desc:'Beschreibung',osc_desc_relay:'Relais N ein- / ausschalten',osc_desc_all:'Alle Relais ein- / ausschalten',osc_desc_ap:'WiFi-Zugangspunkt aktivieren / deaktivieren',osc_desc_reboot:'ESP32 neu starten',osc_custom_note:'Die /relay/N-Adressen können im folgenden Abschnitt angepasst werden.',ap_title:'Wi-Fi / Zugangspunkt (AP)',ap_info:'Der WiFi-AP startet automatisch und schaltet sich nach dem konfigurierten Timeout ab (0 = immer aktiv).',allow_ap:'Zugangspunkt aktivieren',ssid_label:'SSID (WiFi-Netzwerkname)',password:'Passwort (≥8 Zeichen)',ap_ip:'IP des Zugangspunkts',ap_mask:'AP-Maske',ap_gw:'AP-Gateway',ap_timeout:'AP-Ruhezustand (Minuten)',ap_timeout_info:'0 = immer aktiv (kein Ruhezustand)',qr_title:'📱 WiFi-QR-Code',qr_scan:'Mit Ihrem Telefon scannen, um sich zu verbinden',sys_title:'Systeminformationen',model:'Modell:',uptime:'Uptime:',free_ram:'Freier RAM:',min_ram:'Min. RAM:',ethernet_lbl:'Ethernet:',eth_ip:'Ethernet-IP:',osc_port_sys:'OSC-Port:',wifi_ap_lbl:'WiFi AP:',ap_ip_sys:'AP-IP:',ap_clients:'AP-Clients:',ap_sleep_lbl:'AP-Ruhezustand:',active_relays:'Aktive Relais:',cpu_temp:'CPU-Temperatur:',actions:'Aktionen',reboot:'🔄 ESP32 neustarten',factory_reset:'🗑️ Werkseinstellungen zurücksetzen',changelog:'Changelog',relay:'Relais',osc_addr:'OSC-Adresse',invert:'Logik invertieren',mode:'Modus',mode_latch:'Latch (Direktwert)',mode_toggle:'Toggle (Umschaltung)',config_saved:'Konfiguration gespeichert',save_error:'Speicherfehler',net_error:'Netzwerkfehler',net_saved:'Netzwerkkonfiguration gespeichert',net_saved_reboot:'Gespeichert — Neustart in 2s…',ap_saved:'AP-Konfiguration gespeichert',confirm_reboot:'ESP32 neustarten?',rebooting:'Neustart läuft…',confirm_factory:'Werkseinstellungen? Dies kann nicht rückgängig gemacht werden.',resetting:'Zurücksetzen…',log_cleared:'Protokoll gelöscht',connecting:'Verbindung…',connected:'✅ Verbunden',disconnected:'❌ Getrennt',ap_active:'✅ Aktiv',ap_inactive:'⏸️ Inaktiv',disabled_always:'Deaktiviert (immer aktiv)',conn_lost:'Verbindung verloren…',ram_critical:'⚠ Kritischer RAM'},
      zh:{tab_relays:'继电器 & OSC',tab_network:'网络',tab_system:'系统',relay_control:'继电器控制',osc_listen:'OSC 监听 UDP 端口：',all_on:'✅ 全部开启',all_off:'⛔ 全部关闭',relay_osc_config:'继电器 OSC 地址',save_btn:'💾 保存',eth_title:'以太网 (W5500)',use_dhcp:'使用 DHCP',dhcp_info:'启用：自动分配 IP 地址。禁用：使用下方静态字段。',hostname:'主机名 (Hostname)',ip_addr:'IP 地址',subnet:'子网掩码',gateway:'网关',dns1_label:'DNS 1',dns2_label:'DNS 2',static_ip_section:'静态 IP 配置',osc_port:'OSC 监听端口 (UDP)',osc_port_label:'UDP 端口',osc_params_title:'可用 OSC 参数',osc_params_info:'固件接受的 OSC 地址。可接受的值：整数 (i)、浮点数 (f) 或布尔值 (T/F)。',osc_col_addr:'OSC 地址',osc_col_type:'类型',osc_col_values:'可接受值',osc_col_desc:'说明',osc_desc_relay:'启用 / 禁用继电器 N',osc_desc_all:'启用 / 禁用所有继电器',osc_desc_ap:'启用 / 禁用 WiFi 接入点',osc_desc_reboot:'重启 ESP32',osc_custom_note:'/relay/N 地址可在下方章节中自定义。',ap_title:'Wi-Fi / 接入点 (AP)',ap_info:'WiFi AP 自动启动，并在配置的超时后关闭（0 = 始终活动）。',allow_ap:'启用接入点',ssid_label:'SSID（WiFi 网络名称）',password:'密码（≥8 个字符）',ap_ip:'接入点 IP',ap_mask:'AP 掩码',ap_gw:'AP 网关',ap_timeout:'AP 休眠超时（分钟）',ap_timeout_info:'0 = 始终活动（不休眠）',qr_title:'📱 WiFi 二维码',qr_scan:'用手机扫描以连接',sys_title:'系统信息',model:'型号：',uptime:'运行时间：',free_ram:'空闲 RAM：',min_ram:'最低 RAM：',ethernet_lbl:'以太网：',eth_ip:'以太网 IP：',osc_port_sys:'OSC 端口：',wifi_ap_lbl:'WiFi AP：',ap_ip_sys:'AP IP：',ap_clients:'AP 客户端：',ap_sleep_lbl:'AP 休眠：',active_relays:'活动继电器：',cpu_temp:'CPU 温度：',actions:'操作',reboot:'🔄 重启 ESP32',factory_reset:'🗑️ 恢复出厂设置',changelog:'更新日志',relay:'继电器',osc_addr:'OSC 地址',invert:'反转逻辑',mode:'模式',mode_latch:'锁存（直接值）',mode_toggle:'切换',config_saved:'配置已保存',save_error:'保存错误',net_error:'网络错误',net_saved:'网络配置已保存',net_saved_reboot:'已保存 — 2秒后重启…',ap_saved:'AP 配置已保存',confirm_reboot:'重启 ESP32？',rebooting:'正在重启…',confirm_factory:'恢复出厂设置？此操作不可逆。',resetting:'正在重置…',log_cleared:'日志已清除',connecting:'连接中…',connected:'✅ 已连接',disconnected:'❌ 已断开',ap_active:'✅ 活动',ap_inactive:'⏸️ 非活动',disabled_always:'已禁用（始终活动）',conn_lost:'连接丢失…',ram_critical:'⚠ RAM 严重不足'}
    };
    ...
    document.getElementById('saveNet').addEventListener('click', async () => {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          if (txt === 'OK_REBOOT') {
            showMessage('saveNetMsg', '✓ ' + t('net_saved_reboot'));
          } else {
